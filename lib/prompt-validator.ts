 ...
 * IMPORTANTE: La actividad y el género son OPCIONALES - solo el tiempo es obligatorio
 */
export function validatePrompt(prompt: string): ValidationResult {
  ...
  // Detectar si hay un género mencionado (opcional)
  const hasGenre = hasGenreInPrompt(prompt)
  const activities = findMatchingActivities(prompt)
  
  // La actividad y el género son opcionales - no validamos su presencia
